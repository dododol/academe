...
<body>
	<c:choose>
		<c:when test="${not empty result.crudId}">
			<c:set var="actionUrl" value="/letcrud/update.do" />
		</c:when>
		<c:otherwise>
			<c:set var="actionUrl" value="/letcrud/insert.do" />
		</c:otherwise>
	</c:choose>
	
	<form action="${actionUrl}" method="post" name="letcrudVO">		<!-- get가볍다, post 무겁다 --> <!-- name : VO와 맞아야한다. -->
		...
		<c:choose>
			...
			<c:otherwise>
				...
				<a href="/letcrud/selectList.do">취소</a>			
			</c:otherwise>
		</c:choose>
	</form>
</body>
