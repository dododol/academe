	...
	<c:forEach var="fileVO" items="${fileList}" varStatus="status">
		<li>
			<c:choose>
				...
				<c:otherwise>
					<c:url var="downUrl" value="cmm/fms/FileSown.do">
						<c:param name="atchField" value="${fileVO.atchFileId}"/>
						<c:param name="fileSn" value="${fileVO.fileSn}"/>
					</c:url>
					<a href="${downUrl}" download>
						<c:out value="${fileVO.orignFileNm}"/>&nbsp;[<c:out value="${fileVO.fileMg}"/>&nbsp;byte]
					</a>
				</c:otherwise>
			</c:choose>
		</li>
