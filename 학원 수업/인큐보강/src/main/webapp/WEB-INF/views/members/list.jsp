...
<!-- p.98 -->
	...
	<table border="1">
		...
		varStatus : 루프 정보를 담은 객체 활용
		 ...
		 -->
		 <!-- row : membersVO 1행 -->
		<c:forEach var="membersVO" items="${data}" varStatus="stat">
			<tr>
				<td>${stat.count}</td>
				<td><a href="/members/detail?memId=${membersVO.memId}">${membersVO.memId}</a></td>
				<td>${membersVO.memName}</td>
				<td>${membersVO.memHp}></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
	<p>
		<a href="/members/create">회원 등록</a>
	</p>
</body>
</html>
