...
<%
	FanBoardVO fanVo=(FanBoardVO) request.getAttribute("detail");
...
$(function(){
	...
	listBtn.on("click", function(){
		location.href = "<%=request.getContextPath()%>/board/fanboardlist.do";
	});
	
	udtBtn.on("click", function(){
		frm.attr("action", "<%=request.getContextPath()%>/board/fanBoardUpdate.do?bono=<%=fanVo.getB_no()%>");
...
<%--상세보기 시작부분 --%>
	<section class="article-list table-common con">	
      <div class="fanall">
	    <div>
		 <div class="fancon">
		 <div class="fanname2"> <span class="fanname">팬게시판</span></div>
		 <div class="fantitle"> <span><%=fanVo.getB_title()%></span></div>
		  <div class="fantop">
		  <span id="fanid" class="faninfo"> 작성자 </span> <span class="faninfo2"><%=fanVo.getMem_id() %></span>
		  <span class ="gubun">|</span>
		  <span id="fandate" class="faninfo"> 작성일 </span> <span class="faninfo2"> <%=fanVo.getB_date() %></span>
		  <span class ="gubun">|</span>
		   <span id="hitnum" class="faninfo">조회수</span> <span class="faninfo2"> <%=fanVo.getB_hit() %></span>
		  </div>
		       
		        
		   
		        <div id="detailcontent"><span id="con"><%=fanVo.getB_content()%></span></div>
		  </div>
		        <br><br>
		        <div id="detailFileList">
		         ...
		         <tr>
		         	<td class="filetd"><%=fileVO.getOrigin_file_name() %></td>
		       		<td class="filetd"><%=fileVO.getFile_date()%></td>
		            <td class="filetd"><%=fileVO.getFile_size()%> KB</td>
		         </tr>
	    ...
	    <%
	    	if(loginId.getMem_id().equals(fanVo.getMem_id())|| loginId.getMem_id().equals("admin")) {
	    %>
	...
	<form action="<%=request.getContextPath()%>/board/fanBoardDelete.do" method="post" id="frm">
	<input type="hidden" name="bono" value="<%=fanVo.getB_no()%>"/>
	</form>
