...
<%--게시판 시작부분 --%>
	 ...
	 <li><a href="<%=request.getContextPath()%>/board/qnaList.do">문의사항</a></li>
       ...
       <div id="tablediv">
		      <form class="d-flex" action="<%=request.getContextPath()%>/board/fanboardTitleSearch.do" method="post">
			...
			<table id="fanBoard">
				<caption id="fanBoardName">팬게시판 글 목록</caption>
				<thead id="fanthead">
					<tr class="fantr">
				...
				<tbody>
                         	<%	
                        		List<FanBoardVO> fanSearchList =(List<FanBoardVO>)request.getAttribute("search");
                        		
                        		if(fanSearchList== null || fanSearchList.size()==0) {
                        	%>
	                        	...
	                        	<tr>
	                             ...
	                             <%
                        			...
                        			for(FanBoardVO searchVo : fanSearchList) {
                        		%>	
	                               
	                             <tr class="tdtr">
	                                <td class="fantd"><%=searchVo.getB_no() %></td>
	                                <td class="fantd"><a id="tdsub" href="<%=request.getContextPath()%>/board/fanBoardDetail.do?bono=<%=searchVo.getB_no()%>&boid=<%=searchVo.getMem_id()%>"><%=searchVo.getB_title()%></a>
	                                
	                                </td>
	                                <td class="fantd"><%=searchVo.getMem_id()%></td>
	                                <td class="fantd"><%=searchVo.getB_date() %></td>
	                                <td class="fantd"><%=searchVo.getB_hit()%></td>
	                            </tr>	
                        	 	<%	
                        			}
                        		}
