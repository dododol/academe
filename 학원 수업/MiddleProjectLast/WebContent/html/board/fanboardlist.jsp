...
$(function(){
	...
	write.on("click", function(){
		...
		location.href = "<%=request.getContextPath()%>/html/board/fanboardForm.jsp";
	});
...
function proc1() {
	location.href = "<%=request.getContextPath()%>+/board/fanboardTitleSearch.do";
	}
</script>
</head>
...
%>
	...
	<section class="article-list table-common con">
		...
		<div id="tablediv">
			<form
				action="<%=request.getContextPath()%>/board/fanboardTitleSearch.do"
				method="post">
			...
			<table id="fanBoard">
				<caption id="fanBoardName">팬 게시판 </caption>
				<thead id="fanthead">
					<tr class="fantr">
						<th class="fanth" id="thNo">게시글 번호</th>
				...
				<tbody id="tbody">
					<%
						List<FanBoardVO> fanBoardList = (List<FanBoardVO>) request.getAttribute("List");

					if (fanBoardList == null || fanBoardList.size() == 0) {
					%>
					...
					for (FanBoardVO fanVo : fanBoardList) {
					%>

					<tr class="tdtr">

						<td class="fantd"><%=fanVo.getB_no()%></td>
						<td class="fantd" ><a id="tdsub"
							href="<%=request.getContextPath()%>/board/fanBoardDetail.do?bono=<%=fanVo.getB_no()%>&boid=<%=fanVo.getMem_id()%>"><%=fanVo.getB_title()%></a></td>
						<td class="fantd" id="tdid"><%=fanVo.getMem_id()%></td>
						<td class="fantd"><%=fanVo.getB_date()%></td>
						<td class="fantd"><%=fanVo.getB_hit()%></td>
					...
					<%-- <%
						//페이지 처리
					Map<String, Object> pageMap = (Map<String, Object>) request.getAttribute("listvalue");
					...
					<div>
						<ul class="pagination">
							...
							if (i == currentPage) {
							%>
							<li id="" class="page-item active"><a
								class="page-link pageno" href="#"><%=i%></a></li>
							<%
								} else {
							%>
			...
			<input type="button" value="게시글 작성" id="write">
		</div>
