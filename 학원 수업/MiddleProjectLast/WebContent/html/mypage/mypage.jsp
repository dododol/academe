...
%>	
	...
	<header>
	<a id="imga" href="<%=request.getContextPath()%>/mypage/main.jsp"><img src = "<%=request.getContextPath()%>/resources/images/타이틀.png"></a>
		<div id="log1">
		<ul>	
	   <li class="log"><a href="<%=request.getContextPath()%>/logout.do" id="logout">로그아웃</a>
		</li>
			<li class="log"><a href="<%=request.getContextPath()%>/mypage.do" id="ma"><i class="fa fa-user"></i> MY PAGE</a>
		</li>
...
<div class="main">
	...
	<div class="an2">	  
	 ...
	 <li><a href="<%=request.getContextPath()%>응원단.jsp" id="g7">응원단</a>		
      </ul>
	      </div>
		</li>
 ...
 <div class="login-wrap">
  <div class="login-html">
   <!-- <input id="tab-1" type="radio" name="tab" class="sign-in" ><label for="tab-1" class="tab">로그인</label> -->
    <input id="tab-2" type="radio" name="tab" class="sign-up" checked><label for="tab-2" class="tab">회원정보 수정</label>
    <div class="login-form">
      ...
      <div class="sign-up-htm">
        <div class="group">
          <label for="user" class="label">아이디</label>
          <input id="userID" type="text" class="input" placeholder="영문, 숫자로 시작하는 4-8글자를 입력"  name="userID" value="<%=loginInfo.getMem_id()%>" readonly="readonly" style = "color : black; font-weight:bold; ">
          <span id="spanid"></span>
        </div>
        <div class="group">
          <label for="pass" class="label">비밀번호</label>
          <input id="pw" type="password" class="input" name="userPW" data-type="password" placeholder="영문, 숫자로 시작하는 4-8글자를 입력" value="<%=loginInfo.getMem_pw()%>">
        </div>
      
        <div class="group">
          <label for="pass" class="label">이름</label>
          <input id="name" type="text" class="input"  name="userNAME" placeholder="이름 입력" value="<%=loginInfo.getMem_name()%>" readonly style = "color : black; font-weight:bold; ">
        </div>
		  <div class="group">
          <label for="pass" class="label">주민등록번호</label>
          <input id="num" type="text" class="input" name="userNUM" placeholder="-포함해서 입력" value="<%=loginInfo.getMem_num()%>" readonly style = "color : black; font-weight:bold; ">
        </div>
          <div class="group">
		  <label for="pass" class="label">이메일</label> 
		  <input id="mail" type="text" class="input" name="userMAIL" placeholder="@포함해서 입력" value="<%=loginInfo.getMem_mail()%>">
		  </div>
		  <div class="group">
          <label for="pass" class="label">연락처</label>
          <input id="tel" type="text" class="input" name="userTEL" placeholder="-포함해서 입력" value="<%=loginInfo.getMem_tel()%>">
        </div>
		  <div class="group">
          ...
          <input id="sample6_postcode" type="text" class="input" placeholder="우편번호" name="userADDRNUM" value="<%=loginInfo.getMem_addrnum()%>"><br> 
          <input type="text" class="input" id="sample6_address" placeholder="주소" name="userADDR1" value="<%=loginInfo.getMem_addr1()%>"><br> 
          <input type="text" class="input" id="sample6_detailAddress" placeholder="상세주소" name="userADDR2" value="<%=loginInfo.getMem_addr2()%>"><br>
        </div>
		   
		   <div class="group">
    		<input type="submit" class="button" value="수정완료" onclick="return confirm('정말로 수정을 완료하시겠습니까?');">
    		<br>
    		<br>
    		<br>
    		<a href="<%=request.getContextPath()%>/memDelete.do?id=<%=loginId.getMem_id()%>" class="button" style="display: flex; justify-content: center; text-decoration: none; align-items: center; "
    		onclick="return confirm('정말로 회원을 탈퇴하시겠습니까?');">회원 탈퇴</a>
        </div>
		  </form>
