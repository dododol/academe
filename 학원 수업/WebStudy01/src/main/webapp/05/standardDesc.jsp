...
<pre>
	...
	JSP container 동작 단계
	1. jsp템플릿 파일로부터 서블릿 소스 생성
	2. 컴파일
	3. jsp 인스턴스(싱글턴) 생성
	4. requset callback 호출 후 응답 전송...
	
	스펙에 따른 소스 구성 요소
	1. 정적 텍스트, HTML, CSS, JS ... : Front-End 정적 요소
	2. Back-End 요소(script 요소)
		...
		2) directive(지시자) : 실행코드나 응답 컨텐츠를 직접 구성하지 않고, 환경 설정에 사용 &lt;%@page  %&gt;
			- page(required) : jsp 페이지 자체에 대한 설정 변경(속성으로 변경)
					ex) 버퍼의 사용여부, 버퍼의 용량, 세션 사용 여부, el 사용여부 ..
			- taglib(optional)
			- include(optional)
		3) espression(표현식) : <%=today %>
		4) declaration(선언부) : 
		<%! //전역 field나 method 선언 
			public void test(){}
		%>
...
<!-- 			comment1 -->
		...
		8) JSTL(Jsp Standard Tad Library) 
</pre>
